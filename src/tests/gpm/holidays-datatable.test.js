...
import { getHolidaysDatatable } from "../../services/gpm-holidays-datatable.service.js";
...
export function setup() {
  ...
  const page = http.get(`${BASE_URL}/holidays`);
  const match = page.body.match(/<meta[^>]+name="csrf-token"[^>]+content="([^"]+)"|<input[^>]+name="_token"[^>]+value="([^"]+)"/);
  if (!match) {
    throw new Error("No se encontró el _token CSRF en la página de holidays");
  }
...
export default function ({ cookies, csrfToken }) {
  ...
  const res = getHolidaysDatatable(BASE_URL, csrfToken, DT_START, DT_LENGTH, DT_SEARCH);
